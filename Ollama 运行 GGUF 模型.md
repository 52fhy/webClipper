...
有关GGUF类型文件的说明，请看：[GGUF类型模型文件](https://mp.weixin.qq.com/s?__biz=MzkwODQyMzczMg==&mid=2247485299&idx=1&sn=131da81f842b76c5d2d938d8ab1d89b7&scene=21#wechat_redirect)。

我们这里使用: `causallm_7b.Q5_K_S.gguf` 模型文件，作者对这个版本的评价是： `large, low quality loss - recommended`。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/pOCgGjjG1QkvneGmJFZygrvjicLjcrxic4DMHyHdJVuocPbxMd5Ef8NTIvkco5mQvrL3yhHA5OTMJEYtaZeNQ1Vw/640?wx_fmt=png&from=appmsg)
...
```
cd \github\ollamaollama create c7b -f ./causallm7bq5.mf
```
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/pOCgGjjG1QkvneGmJFZygrvjicLjcrxic4OX9MaJdaLu6gmO8RkD5zFv1JpTLWmXNbxNkBRFlmVnlgeOwLicWdRAw/640?wx_fmt=png&from=appmsg)
...
```
# Modelfile generated by "ollama show"# To build a new Modelfile based on this one, replace the FROM line with:# FROM gemma:2bFROM C:\Users\Administrator\.ollama\models\blobs\sha256-c1864a5eb19305c40519da12cc543519e48a0697ecd30e15d5ac228644957d12TEMPLATE """<start_of_turn>user{{ if .System }}{{ .System }} {{ end }}{{ .Prompt }}<end_of_turn><start_of_turn>model{{ .Response }}<end_of_turn>"""PARAMETER repeat_penalty 1PARAMETER stop "<start_of_turn>"PARAMETER stop "<end_of_turn>"
```
...
```
FROM <model name>:<tag>FROM llama2FROM ./ollama-model.bin
```
